...
// Banner (superblock) lives in its own sector, ahead of the directory table,
// so rewriting it never risks touching directory or file data. 16 pages per
// sector = 16 rotating copies before an erase is needed.
#define BANNER_START 0
#define BANNER_SIZE SECTOR_SIZE
#define BANNER_PAGES (BANNER_SIZE / PAGE_SIZE)  // 16
#define BANNER_MAGIC 0x46415431UL               // "FAT1"
...
// ========== FAT BANNER ===============================
// One instance is written per page (16 copies per sector). fs_sync() appends
// a new copy on shutdown; fat_initailizing() reads all 16 and trusts the one
// with the highest 'sequence' that also passes its own CRC check.
struct __attribute__((packed)) FatBanner {
  uint32_t magic;                // BANNER_MAGIC - "is this our filesystem"
  uint8_t version;                // BANNER_VERSION - layout version
  uint32_t sequence;              // increasing counter across all copies ever written
  uint8_t chip_id[3];             // flash_read_id() result, captured at format time
  uint32_t next_free_address;     // cached allocator state
  uint16_t used_entries;          // file count, cheap sanity check vs a full scan
  uint16_t next_free_dir_index;   // hint for fs_create_file()'s free-slot search
  uint8_t reserved[32];           // room for future fields without changing struct size
  uint32_t crc;                   // CRC32 over every field above this one
};
// ====================== GLOBALS ======================
...
bool has_file = false;  // true once at least one file exists; disambiguates
                        // "no files yet" from "last file's entry is at dir addr 0"
...
// Used only for validating/stamping FatBanner copies - infrequent enough
// (once at boot, once at shutdown) that the table-free version's extra
// cycles don't matter, and it avoids spending RAM/flash on a lookup table.
...
// Erases every sector overlapping [addr, addr+len). NOR flash can only clear
// bits 1->0 when programming - it can never set them back to 1 without an
// erase - so anything about to be programmed must be erased first unless
// it's known-blank space from a full-chip erase.
...
void fs_format() {
  // Previously this only erased the directory table (addr < DIR_TABLE_SIZE),
  // never the data region. Since NOR flash can't be programmed back to 1s
  // without an erase, any leftover data-region content from before this
  // format would have caused fs_write2_file() to silently corrupt writes.
  flash_chip_erase();
...
// Rebuilds next_free_address / address_last_file / has_file from what's
// actually on the chip. Must be called at boot - without it, a power cycle
// forgets every file that was created in a previous session and the
// allocator starts handing out addresses that are already in use.
void fs_scan_directory() {
  ...
  for (int i = 0; i < DIR_ENTRIES; i++) {
    ...
    uint32_t start = flash_read_u32(addr + 23);
    uint32_t size = flash_read_u32(addr + 27);
...
bool fs_create_file(const char *name, uint32_t size) {
  ...
  // Erase the region this file will occupy before writing into it. On a
  // freshly-formatted chip this is already blank, but once compaction/reuse
  // of freed space exists, this is what keeps writes from corrupting on top
  // of old data.
...
// Scans all 16 banner-sector pages. Returns true and fills *out with the
// valid copy that has the highest sequence number. *out_sector_blank comes
// back true only if every page is still fully erased (0xFF) - i.e. fs_sync()
// has never run since the last full-chip/banner-sector erase.
bool fs_read_latest_banner(FatBanner *out, bool *out_sector_blank) {
  bool found = false;
  uint32_t best_sequence = 0;
  bool sector_blank = true;

  for (uint16_t page = 0; page < BANNER_PAGES; page++) {
...
// Persists current live filesystem state into the banner. Call this
// explicitly at controlled shutdown - NOT after every file operation, since
// each call costs a page write and, once every 16 calls, a sector erase.
void fs_sync() {
  FatBanner latest;
  bool sector_blank;
  ...
  // Find the next unused page. Writes always go in order, so a page's first
  // byte still being 0xFF (erased) is enough to know it's unused.
  int free_page = -1;
  for (uint16_t page = 0; page < BANNER_PAGES; page++) {
    uint32_t addr = BANNER_START + (uint32_t)page * PAGE_SIZE;
  ...
  if (free_page < 0) {
    // All 16 slots used - wrap around. sequence keeps counting up so a
    // pre-erase copy could never look newer than what we write next, even
    // if one somehow survived (it won't, but the invariant costs nothing).
...
bool fat_initailizing() {
  ...
  if (valid) {
    // Known filesystem. Trust the cached state as a fast path, but still run
    // a directory scan as a correctness net - if a crash happened between a
    // file write and the last fs_sync(), the banner can be slightly stale;
    // the scan recomputes the real allocator state from the directory table.
    fs_scan_directory();
    return true;
  }

  if (sector_blank) {
    // No banner has ever been written - genuinely new/unformatted chip.
    // Auto-format is safe here because there's no existing data to lose.
    fs_format();
    return true;
  }

  // Something occupies the banner sector that isn't blank and doesn't
  // validate (wrong magic/version, failed CRC, or foreign data). This could
  // be real data from a different filesystem or corruption of our own -
  // either way, auto-formatting would silently destroy it. Refuse to mount.
  return false;
}
